...
group_name = "ББИ-24-3"
sheet_name = "1 курс"
schedule_file_url = "https://misis.ru/files/-/0424075039793adc358671aad2b25b50/ikn_050924.xlsx"
...
def create_event(cal, subject, location, start, end):
    event = Event()
...
for index, row in df_upper_week.iterrows():
    if pd.notna(row['Занятие']):  # Проверяем, что занятие не пустое
        ...
        create_event(cal, row['Занятие'], row['Аудитория'], start, end)
